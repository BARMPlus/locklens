...
function normalizeVulnerabilities(advisories: PackageAuditAdvisory[]): VulnerabilityCounts {
  const counts = {
    info: 0,
    low: 0,
    moderate: 0,
...
function normalizeThresholdSeverities(threshold: AuditThreshold): AuditThreshold[] {
...
export function normalizeMetadata(
  ...
  const optionalDependencies = metadata ? readNumber(metadata, 'optionalDependencies') : null
  ...
  return {
    ...
    dependencies:
      dependencies ??
      (dependencyContainer ? readNumber(dependencyContainer, 'dependencies') : null),
    devDependencies:
      devDependencies ??
      (dependencyContainer ? readNumber(dependencyContainer, 'devDependencies') : null),
    optionalDependencies:
      optionalDependencies ??
      (dependencyContainer ? readNumber(dependencyContainer, 'optionalDependencies') : null),
    totalDependencies:
      totalDependencies ?? (dependencyContainer ? readNumber(dependencyContainer, 'total') : null),
  }
}
