...
function normalizeAdvisoriesMap(
  ...
  for (const [key, value] of Object.entries(advisoryMap)) {
    ...
    advisory.severity = isSupportedSeverity(value.severity) ? value.severity : advisory.severity
...
function normalizeVulnerabilitiesMap(
  ...
  for (const [moduleName, value] of Object.entries(vulnerabilities)) {
    ...
    for (const via of value.via) {
      ...
      advisory.severity = isSupportedSeverity(via.severity) ? via.severity : advisory.severity
      advisory.moduleName = readString(via, 'name') ?? moduleName ?? advisory.moduleName
...
export function normalizeAdvisories(context: NormalizerContext): PackageAuditAdvisory[] {
  ...
  if (isRecord(context.auditPayload)) {
    ...
    normalizeAdvisoriesMap(advisories, context.auditPayload, pathMap, allowlistedIds)
    normalizeVulnerabilitiesMap(advisories, context.auditPayload, pathMap, allowlistedIds)
  }
...
export function filterAdvisoriesByThreshold(
  ...
  return advisories.filter((advisory) => severityMeetsThreshold(advisory.severity, threshold))
}
