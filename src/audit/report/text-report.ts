...
const SEVERITY_ORDER: SupportedSeverity[] = ['critical', 'high', 'moderate', 'low', 'info']

const SUMMARY_SEVERITY_ORDER: SupportedSeverity[] = ['critical', 'high', 'moderate', 'low']
...
function getSeverityLabel(severity: SupportedSeverity | null, language: AuditOutputFormatLanguage) {
...
function buildAdvisoryView(
  ...
  return {
    ...
    titleLine: advisory.title ?? (language === 'en' ? 'Title not available' : '未返回标题'),
...
function buildAdvisoryGroups(result: PackageAuditResult, language: AuditOutputFormatLanguage) {
  const severityLabels = getSeverityLabels(language)
...
function buildDisplayedSeverityLabels(
  ...
  const severityLabels = getSeverityLabels(language)
  const labels = result.metadata.thresholdSeverities.map((severity) => severityLabels[severity])
