...
import type { LockfileDetectionResult, LockfileName, PackageManager } from './types'
...
export async function detectLockfile(directory: string): Promise<LockfileDetectionResult> {
  ...
  const detectedLockFiles = checks.filter((item) => item.exists).map((item) => item.lockfileName)
