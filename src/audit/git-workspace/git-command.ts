...
const NON_INTERACTIVE_SSH_COMMAND = 'ssh -oBatchMode=yes -oStrictHostKeyChecking=accept-new'
...
export async function runGitCommand(args: string[], options: GitCommandOptions) {
  try {
    ...
    throw buildGitCommandError(options, args, stderr.trim() || stdout.trim(), error)
  }
}
