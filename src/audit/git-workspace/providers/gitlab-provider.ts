...
import { isGitLabHost, parseRemoteRepositoryLocator, type RemoteWorkspaceProvider } from './shared'
...
export const gitlabProvider: RemoteWorkspaceProvider = {
  ...
  isConfigured: (env = process.env) => Boolean(env[LOCKLENS_GITLAB_TOKEN_ENV]?.trim()),
  buildPlan: (source) => buildGitLabProviderPlan(source, 'gitlab', LOCKLENS_GITLAB_TOKEN_ENV),
  fetchWorkspace: async (source, env = process.env) =>
    fetchWorkspaceThroughGitLabApi(source, 'gitlab', LOCKLENS_GITLAB_TOKEN_ENV, env),
}
