...
export const LOCKLENS_GITLAB_PRIVATE_TOKEN_ENV =
  "LOCKLENS_GITLAB_PRIVATE_TOKEN" as const;
...
export const gitlabSelfManagedProvider: RemoteWorkspaceProvider = {
  ...
  matches: (source: ResolvedRemoteAuditSource) =>
    ...
    isGitLabSelfManagedCandidateHost(
      parseRemoteRepositoryLocator(source.inputSource).hostname
    ),
  isConfigured: (env = process.env) =>
    Boolean(env[LOCKLENS_GITLAB_PRIVATE_TOKEN_ENV]?.trim()),
  buildPlan: (source) =>
    buildGitLabProviderPlan(
      source,
      "gitlab-self-managed",
