...
function getAuditor(packageManager: PackageManager) {
  ...
  switch (packageManager) {
    ...
    case 'yarn':
      return yarnAudit
    default:
      throw new AuditExecutionError(`Unsupported package manager: ${packageManager}`)
  }
}
...
export async function runLibraryAuditAdapter(
  ...
  const reporter = (summary: AuditCiSummary, _config: unknown, audit?: unknown): AuditCiSummary => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'audit-ci execution failed'
